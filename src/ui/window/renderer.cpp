...
namespace dune {
    namespace ui {
        ...
        void Renderer::init_buffers() {
            back_buffer.resize(height, std::vector<char>(width, ' '));
            front_buffer.resize(height, std::vector<char>(width, ' '));
        ...
        void Renderer::draw_string(int x, int y, const std::string& str, int color) {
        ...
        void Renderer::draw_border(int x, int y, int width, int height) {
            for (int i = x; i < x + width; ++i) {
                draw_char(i, y, '#');
                draw_char(i, y + height - 1, '#');
            }
            for (int i = y; i < y + height; ++i) {
                draw_char(x, i, '#');
                draw_char(x + width - 1, i, '#');
            }
        }
        ...
        char Renderer::get_char_at(int x, int y) const {
            return is_valid_position(x, y) ? back_buffer[y][x] : ' ';
        }
